class Admin::FaqCategoriesController < Admin::AdminController
  ...
  def index
    ...
    @faq_category_count = FaqCategory.count
    @faq_categories = FaqCategory.page(params[:page]).per(params[:per_page])
  end
  ...
  def create
    ...
    respond_to do |format|
      if @faq_category.save
        format.html { redirect_to admin_faq_category_path(@faq_category), notice:  t(:faq_category, scope: [:activerecord, :models])+t(:message_success_create) }
  ...
  def update
    respond_to do |format|
      if @faq_category.update(faq_category_params)
        format.html { redirect_to admin_faq_category_path(@faq_category), notice:  t(:faq_category, scope: [:activerecord, :models])+t(:message_success_update) }
